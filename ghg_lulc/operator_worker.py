...
import pandas as pd
from climatoology.base.artifact import _Artifact, RasterInfo
from climatoology.base.operator import Operator, Info, PluginAuthor, Concern
...
class GHGEmissionFromLULC(Operator[ComputeInput]):
    ...
    def info(self) -> Info:
        ...
        return Info(name='LULC Change Emission Estimation',
                    ...
                    authors=[PluginAuthor(name='Veit Ulrich',
                             ...
                             PluginAuthor(name='Moritz Schott',
    ...
    def compute(self, resources: ComputationResources, params: ComputeInput) -> List[_Artifact]:
        ...
        """
        emission_calculator = EmissionCalculator(emission_factors=self.emission_factors[params.ghg_stock_source],
                                                 resources=resources)
        ...
        table_artifacts = create_table_artifacts(emission_calculator,
                                                 emissions_df,
    ...
    def get_changes(self,
                    ...
                    params: ComputeInput,
                    resources: ComputationResources) -> Tuple[gpd.GeoDataFrame, List[_Artifact]]:
        lulc_before, lulc_after = self.get_classifications(params)

        classification_artifacts = create_classification_artifacts(lulc_before,
                                                                   lulc_after,
                                                                   resources)

        change_raster, change_emissions_raster = emission_calculator.derive_lulc_changes(lulc_before,
                                                                                         lulc_after)
    ...
    def get_classifications(self, params: ComputeInput) -> Tuple[RasterInfo, RasterInfo]:
        ...
        area_before = LulcWorkUnit(area_coords=aoi_box,
                                   end_date=params.date_before,
                                   threshold=params.classification_threshold)
        area_after = LulcWorkUnit(area_coords=aoi_box,
...
def create_table_artifacts(emission_calculator: EmissionCalculator,
                           ...
                           params: ComputeInput,
                           resources: ComputationResources) -> List[_Artifact]:
...
def create_chart_artifacts(emissions_df: gpd.GeoDataFrame,
